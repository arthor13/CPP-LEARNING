    ...
    */
    ...
    /*
        Vector em C++
            ...
            Para aumentar (tipo append em python):
    ...
    */
    ...
    /*
        Estruturas condicionais
            IF, ELSE e  ELSE IF são iguais ao C. NÃO TEM ERRO
                IF(CONDIÇÃO){}
                ELSE IF{}
                ELSE{}
            SWITCH IGUAL AO C
        Estruturas de repetição
            FOR É IGUAL AO C. NÃO TEM ERRO
                FOR(INICIALIZAÇÃO; CONDIÇÃO; INCREMENTO){}
            OBS: DIFERENTE DE ALGUMAS VERSÕES DE C É POSSÍVEL DECLARAR VARIÁVEIS NA ESTRUTURA
            for(array_type X : array_name) -> FOREACH
                O MESMO FUNCIONA EM VECTOR
            JEITO DE DAR LOOP EM VECTOR UTILIZANDO ITERADOR
                for (std::vector<int>::iterator itr = idsVec.begin(); itr != idsVec.end(); itr++)
                    std::cout << *itr << std:endl;

            WHILE E DO WHILE IGUAL AO C

            ENUM É IGUAL À C
            EXISTE ENUM CLASS:
                enum class nome_do_enum
                {
                    Nome,
                    nome
                };
            AO CHAMAR E ASSOCIAR UM DADOD:
                nome_do_enum variavel = nome_do_enum::Nome;
            ELE NÃO FAZ CONVERSAO AUTOMATICA PRA INTEIRO QUE NEM ENUM PADRAO.
            É PRECISO FAZER CONVERSAO EXPLICITA PRO TIPO DA ENUM OU PRO TIPO INTEIRO
                int variavel = (int) variavelEnum;s
                nomeDoEnum nomeVariavel = (nomeDoEnum)0;
        */
}